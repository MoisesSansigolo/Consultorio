...
namespace Consultorio.Map
{
    public class ProfissionalMap : BaseMap<Profissional>
    {
        ...
        public override void Configure(EntityTypeBuilder<Profissional> builder)
        {
            ...
            builder.HasMany(x => x.Especialidades).WithMany(x => x.Profissionais).UsingEntity<ProfissionalEspecialidade>(x =>
            x.HasOne(p => p.Especialidade).WithMany().HasForeignKey(x => x.EspecialidadeId),
